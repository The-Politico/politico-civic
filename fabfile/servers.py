...
def checkout_latest(remote="origin"):
    ...
    with cd("%(SERVER_PROJECT_PATH)s" % server_config.__dict__):
        run(
            "cd %s; git fetch %s" % (server_config.SERVER_PROJECT_PATH, remote)
        )
        run(
            "cd %s; git checkout %s; git pull %s %s"
            % (
                server_config.SERVER_PROJECT_PATH,
                env.branch,
                remote,
                env.branch,
            )
    ...
    with settings(warn_only=True):
        for service, remote_path, extension in server_config.SERVER_SERVICES:
            ...
            if a != b:
                ...
                if service == "nginx":
                    sudo(
                        "rm /etc/nginx/sites-enabled/%s.nginx.conf"
                        % server_config.PROJECT_FILENAME
                    )
                    sudo(
                        "ln -s /etc/nginx/sites-available/%s.nginx.conf"
...
def fabcast(command):
    ...
    """
    run(
        "cd %s && bash run_on_server.sh pipenv run fab %s %s"
